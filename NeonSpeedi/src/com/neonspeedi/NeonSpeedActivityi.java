...
public class NeonSpeedActivityi extends Activity implements View.OnClickListener
{
    private String version = " ARM/Intel NeonSpeed Benchmark V1.2 ";
    ...
    static 
    {
        System.loadLibrary("neonspeedilib");
    }
            ...
            builder.setPositiveButton("About Summary", new DialogInterface.OnClickListener() 
            {
                public void onClick(DialogInterface dialog, int which) 
                {
                    mDisplayDetails.setText(
                      ...
                      " single precision floating point with x[m]=x[m]+s+y[m] \n" + 
                      " and x[m]=x[m]+y[m] using integers. Million Floating  \n" + 
                      " Point Operations Per Second (MFLOPS) speed can be    \n" +                     
                      " calculated by dividing single precision MB/second by \n" +
                      " 4 and 8, for the two tests. Cache sizes are indicated \n" +
                      " are indicated by varying performance as memory usage \n" +
                      " changes. The first set of tests are run using normal \n" +
                      " compilations then via NEON intrinsic functions, with \n" +
                      " the second set only NEON. Test functions are the same \n" +
                      " as used in the MemSpeed benchmark, except those using \n" +
                      " double precision floating point, unsupported in NEON. \n" +
                      " \n" +
                      " Roy Longbottom 2013\n");
                    resetTest();
                    testDone = 0;
 ...
 */
              ...
              builder2.setNegativeButton("OK", new DialogInterface.OnClickListener() 
              {
                  public void onClick(DialogInterface dialog, int which) 
                  {
                    Editable value = input.getText();
                        ...
                        emailIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, "ARM/Intel NeonSpeed Benchmark Results");
    ...
    public void clear()
    {
        xout[0]  = version + date$ + "\n" + stringFromJNI2();
        xout[1]  = "\n";
        xout[2]  = "       Vector Reading Speed in MBytes/Second\n";
        xout[3]  = "  Memory  Float v=v+s*v  Int v=v+v+s   Neon v=v+v\n";
        xout[4]  = "  KBytes   Norm   Neon   Norm   Neon  Float    Int\n";
        xout[5]  = "\n";
        xout[6]  = "\n";
        xout[7]  = "\n";
        xout[8]  = "\n";
        ...
        xout[13] = "\n";
        xout[14] = "\n";
        xout[15] = " Running time 10 to 20 seconds ARMv7 with NEON\n"; 
        xout[16] = "\n"; 
        testDone = 0;       
    }

...
public class MPFPUStressActivity extends Activity 
{
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        params = String.format(" params %d thrds, %d ops/wd, " + size$ + ", %d mins\n", thrdNumber, opwdNumber, minNumber);
    ...
    {
        outputText.setText("The arithmetic operations executed are of the form x[i] = (x[i] + a) * b - (x[i] + c) * d + (x[i] + e) * f " +
"with 2, 8 and 32 operations per input data word, using 1, 2, 4, 8, 16 and 32 threads. Data sizes are limited to three to use L1 cache, " + 
"L2 cache and RAM at 12.8, 128 and 12800 KB.  The program checks for consistent numeric results, primarily to show " + 
"that all calculations are carried out and can be run. If an error is found a result of zero is reported.\n\n" + 
...
"The tests can increase CPU temperatures and lead to slower speed and eventual power down. Test sessions of no more than 15 minutes are recommended.\n\n");
    ...
    protected Dialog onCreateDialog(int id) 
    {
        switch (id) 
        {
          case 1:
              ...
              builder1.setMessage("Enter Ops per word - 2, 8 or 32");
              ...
              builder1.setNegativeButton("OK", new DialogInterface.OnClickListener() 
              {
                  ...
                  try 
                  {
                      testNumber = Integer.parseInt(input1.getText().toString());
                  } 
                      catch(NumberFormatException e) 
                  {
                      Toast.makeText(getApplicationContext(),"Invalid Number",Toast.LENGTH_LONG).show();
                  }
                  if (testNumber == 2 || testNumber == 8 || testNumber == 32) 
                  {
                       opwdNumber = testNumber;
                       params = String.format(" params %d thrds, %d ops/wd, " + size$ + ", %d mins\n", thrdNumber, opwdNumber, minNumber);
                  ...
                  {
                     String amessage = String.format(" Invalid Ops per word, still %d\n", opwdNumber) + params;
                     Toast.makeText(getApplicationContext(), amessage, Toast.LENGTH_LONG).show();
                     outputText.setText(amessage);
                  }
              } 
          ...
          case 3:
              ...
              builder3.setNegativeButton("OK", new DialogInterface.OnClickListener() 
              {
                  ...
                  {
                       kbNumber = testNumber;
                       if (kbNumber ==1) size$ = " 12.8 KB"; 
                       if (kbNumber ==2) size$ = "  128 KB"; 
                       if (kbNumber ==3) size$ = " 12.8 MB"; 
                       params = String.format(" params %d thrds, %d ops/wd, " + size$ + ", %d mins\n", thrdNumber, opwdNumber, minNumber);
                       String amessage = String.format(params);
                       Toast.makeText(getApplicationContext(), amessage, Toast.LENGTH_LONG).show();
                       outputText.setText(amessage);
              ...
              builder9.setNegativeButton("OK", new DialogInterface.OnClickListener() 
              {
                  public void onClick(DialogInterface dialog, int which) 
                  {
                    Editable value = input.getText();
                        ...
                        emailIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, "Android MP-FPU-Stress Results");
    ...
    public native String  stringFromJNI(int threads, int opwd, int stressit, int kbCode, int mult);

    public void testCPU()
    {
        startTime = System.currentTimeMillis();

        x0 = stringFromJNI(threads, opwd, stressit, kbCode, mult);
    ...
    private class RunTests extends AsyncTask<Void, Integer, Void>
    {
        ...
        {
            for(opwd=2; opwd<33; opwd=opwd*4)
            {
                for(threads=1; threads<10; threads=threads*2)
                {
                        testCPU();

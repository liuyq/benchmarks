...
public class Dhrystone2iActivity extends Activity  implements View.OnClickListener
{
 
    private String version = " ARM/Intel Dhrystone 2 Benchmark 1.2 ";
    ...
    {  
        ...
        {
             clear();
            _myTask = new myDhry2Task();  
            _myHandler = new Handler();  
            xout[8] = " Running Time less than 2 seconds\n";
            displayAll();  
          ...
          case 8:
            ...
            builder.setPositiveButton("About Summary", new DialogInterface.OnClickListener() 
            {
                public void onClick(DialogInterface dialog, int which) 
                {
                    ...
                    " The Dhrystone Benchmark provides a measure of integer\n" + 
                    " performance. It became the key standard benchmark    \n" +  
                    " from 1984, with the growth of Unix systems. The first\n" + 
                    " version was produced by Reinhold P. Weicker in ADA\n" + 
                    " and translated to C by Rick Richardson. Two versions\n" + 
                    " are available. This second one was produced to avoid\n" + 
                    " over-optimisation problems encountered with version 1.\n" +                     
                    " Originally, the benchmark gave performance ratings in\n" +
                    " terms of Dhrystones per second. This was later changed \n" +
                    " to VAX MIPS by dividing Dhrystones per second by 1757, \n" +
                    " the DEC VAX 11/780 result.\n" +
            ...
            builder.setNegativeButton("Dhrystone Results on PCs", new DialogInterface.OnClickListener() 
            {
                public void onClick(DialogInterface dialog, int which) 
                {
                   Uri uri = Uri.parse("http://www.roylongbottom.org.uk/dhrystone results.htm"); 
                   startActivity(new Intent(Intent.ACTION_VIEW, uri));
              ...
              builder2.setNegativeButton("OK", new DialogInterface.OnClickListener() 
              {
                  public void onClick(DialogInterface dialog, int which) 
                  {
                    Editable value = input.getText();
                        ...
                        emailIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, "Android Dhrystone 2 Benchmark Results");
    ...
    private void displayAll()
    {
        if (testDone == 0)
        {
         ...
         {
             mDisplayDetails.setText(xout[0]  + xout[1]  + x0 + xout[5] +
                                         xout[6] + xout[7] + xout[8] + 
                                         xout[9] + xout[10] + xout[11] +
                                         xout[12]  + xout[13] + xout[14]);
         }
      }
    ...
    private class myDhry2Task implements Runnable  
    {  
        ...
        public void run() 
        {
            ...
            xout[8] = " Total Elapsed Time  " + String.format("%5.1f", testTime)
                    + " seconds\n";
            testDone = 1;
            displayAll();
            resetTest();          
    ...
    public void clear()
    {
        xout[0]  = version + date$ + "\n" + stringFromJNI2();
        xout[1]  = "\n";
        xout[2]  = " Nanoseconds one Dhrystone run\n";
        xout[3]  = " Dhrystones per Second\n";
        xout[4]  = " VAX MIPS rating\n";
        xout[5]  = "\n";
        xout[6]  = "\n";
        xout[7]  = "\n";
        xout[8]  = "\n";
